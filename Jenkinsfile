pipeline {
    ...
    parameters {
	    //git代码路径【参数值对外隐藏】
	    string(name:'repoUrl', defaultValue: 'git@github.com:XGWang0/jenkins_test_repo.git', description: 'git代码路径 only for jenkins pipeline test')
	    ...
	    //Config file path 相对路径
	    string(name:'cfgPath', defaultValue: 'config.xml', description: 'config的相对路径')

	    //executale file 的相对路径
	    string(name:'objectFile', defaultValue: 'test.sh', description: 'executable file的相对路径 ')
    ...
    */
    ...
    environment{
        ...
        CRED_ID='git_passwd'
        //测试人员邮箱地址【参数值对外隐藏】
        QA_EMAIL='xgwang@suse.com'
        //接口测试（网络层）的job名，一般由测试人员编写
        ITEST_JOBNAME='jenkins test'
    }
    options {
        //保持构建的最大个数
